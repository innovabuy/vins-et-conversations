...
export default function PrestationEcolesPage() {
  ...
  const heroBg = useSiteImage('ecoles_hero');
  const gal1 = useSiteImage('ecoles_galerie_1');
  const gal2 = useSiteImage('ecoles_galerie_2');
  ...
  return (
    <div>
      <section
        className={`relative text-white py-20 ${heroBg?.image_url ? 'bg-emerald-900' : 'bg-gradient-to-br from-emerald-800 via-emerald-700 to-emerald-900'}`}
          ...
          <div className="mb-12">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {galleryItems.map((img, i) => (
                <figure key={i} className="rounded-2xl overflow-hidden bg-emerald-50 shadow-sm">
                  <img
                    src={img.image_url}
                    alt={img.alt_text || `Intervention école ${i + 1}`}
                    loading="lazy"
